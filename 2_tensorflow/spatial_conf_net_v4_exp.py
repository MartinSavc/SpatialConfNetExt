...
class KeyPointSpatialConfNetV4(KeyPointSpatialConfNet):
    def __init__(self):
        super().__init__()

        self.name = 'spatial-config-net-v4'
    ...
    def init_model(self):
        ...
        for n in range(self.net_count):
            ...
            spc_map = self.init_spatial_configuration_layers(la_map, N, None)

            detect_map = keras.layers.Add()([la_map, spc_map])

            sm_detect_map = spatialsoftmax.SpatialSoftmax()(detect_map)
            pts_feat_part = spatialsoftmax.GeometricMean(normalize=False)(sm_detect_map)
            pts_feat_list.append(pts_feat_part)
        pts_feat = keras.layers.Concatenate()(pts_feat_list)
        pts_feat_lin = keras.layers.Reshape((2*N*self.net_count,))(pts_feat)

        pts_target_lin = keras.layers.Dense(
                2*N,
                activation='linear',
                kernel_regularizer=keras.regularizers.l1(self.l2_weight),
                )(pts_feat_lin)
        pts_target = keras.layers.Reshape((2, N))(pts_target_lin)
    ...
    def preview(self, data):
        ...
        for image_patch, kp_ref in predict_data_seq:
            ...
            pts_output = self.model.predict(image_patch)
