...
class KeyPointSpatialConfNetV52(KeyPointSpatialConfNet):
    def __init__(self):
        super().__init__()

        self.name = 'spatial-config-net-v52'
    ...
    def init_model(self):
        ...
        for n in range(self.net_count):
            ...
            spc_submap = self.init_spatial_configuration_layers(la_submap, N, 'tanh', not self.global_prior)
    ...
    def preview(self, data):
        ...
        for (image_patch, kp_ref), _target_loss in predict_data_seq:
            ...
            ax_table[1].imshow(output_map[0].max(2))

            def on_pick_event(event, ax_table, output_map):
                i = event.ind[0]
                ax_table[1].cla()
                ax_table[1].imshow(output_map[0, :, :, i])
...
if __name__ == '__main__':
    KeyPointSpatialConfNetV52.main()
